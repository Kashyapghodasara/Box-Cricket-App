...
const chartConfig = {
  Small: { label: "Small", color: "var(--chart-1)" },
  Medium: { label: "Medium", color: "var(--chart-2)" },
  Large: { label: "Large", color: "#9876FF" },
}
  ...
  if (viewBox && "cx" in viewBox && "cy" in viewBox) {
    ...
    return (
      <text x={viewBox.cx} y={viewBox.cy} textAnchor="middle">
        <tspan x={viewBox.cx} y={(viewBox.cy || 0) - 16} className="fill-[#dbfff5] text-2xl font-bold tracking-wider" >
...
const BoxStatGraph = () => {
  ...
  if (!chartData || chartData.length === 0) {
    return (
      <Card><CardHeader><CardTitle>Box-wise Bookings</CardTitle></CardHeader><CardContent><div>Loading stats...</div></CardContent></Card>
    )
  }

  return (
    <Card className="flex flex-col bg-transparent">
      <CardHeader className="items-center pb-0"><CardDescription>January - June 2024</CardDescription></CardHeader>
      <CardContent className="flex flex-1 items-center pb-0">
        <ChartContainer config={chartConfig} className="mx-auto aspect-square w-full max-w-[250px]">
          <RadialBarChart data={chartData} endAngle={180} innerRadius={80} outerRadius={200}>
            <ChartTooltip cursor={false} content={<ChartTooltipContent hideLabel />} />
            <PolarRadiusAxis tick={false} tickLine={false} axisLine={false}>
              {/* ✅ THE FIX IS HERE - Passing the data prop */}
              <Label content={<CenterLabel data={chartData} />} />
            </PolarRadiusAxis>
            <RadialBar dataKey="Small" stackId="a" cornerRadius={5} fill="var(--color-Small)" className="stroke-transparent stroke-2" />
            <RadialBar dataKey="Medium" fill="var(--color-Medium)" stackId="b" cornerRadius={5} className="stroke-transparent stroke-2" />
            <RadialBar dataKey="Large" fill="var(--color-Large)" stackId="c" cornerRadius={5} className="stroke-transparent stroke-2" />
          </RadialBarChart>
        </ChartContainer>
      </CardContent>
    </Card>
